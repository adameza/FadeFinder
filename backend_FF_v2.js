class BarberFinder 
{
  constructor() {
    this.barberLocations = [
      ...
      { name: "Barber 2", lat: 40.7128, lng: -74.0060 },
...
cancelAppointment(appointment)
{
    ...
    if(this.waitingList.length > 0)
    {
        ...
        editAppointment(appointment, row) {
          ...
          if (currentDate !== appointment.date || currentTime !== appointment.time) {
            ...
            sendNotification(appointment.client.email, `Your appointment on ${appointment.date} at ${appointment.time} has been changed.`);
          }
        }
    }
    ...
    document.getElementById("filter-appointments-form").addEventListener("submit", (event) => {
    ...
    const appointmentIndex = parseInt(event.target.getAttribute("data-index"), 10);  
