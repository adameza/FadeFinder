class FadeFinder {
	...
	scheduleAppointment(clientName, clientEmail, barberName, appointmentDate, appointmentTime) {
		...
		const appointment = { clientName: clientName, barberName: barberName, appointmentDate: appointmentDate, appointmentTime: appointmentTime, };
	...
	sendConfirmationEmail(email, appointment) {
		...
		console.log(`Your appointment with ${appointment.clientName} on ${appointment.appointmentDate.toLocaleDateString()} at ${appointment.appointmentTime} has been successfully scheduled.`);
	...
	isBarberAvailable(barberName, appointmentDate, appointmentTime) {
		...
		for (const appointment of this.appointments) {
			if (appointment.barberName === barberName && appointment.appointmentDate.getTime() === appointmentDate.getTime() && appointment.appointmentTime === appointmentTime) {
				return false;
			}
		}
		return true;
	...
	getAppointmentsByBarber(barberName) {
		return this.appointments.filter(appointment => appointment.barberName === barberName);
	}

	getAppointmentsByDate(appointmentDate) {
		return this.appointments.filter(appointment => appointment.appointmentDate.getTime() === appointmentDate.getTime());
	}

	getAppointmentsByClient(clientName) {
		return this.appointments.filter(appointment => appointment.clientName === clientName);
	}
	...
	sendingAppointmentReminders() {
		...
		const upcomingAppointments = this.appointments.filter(appointment => {
			...
			return appointmentDate > now && appointmentDate < new Date(now.getTime() + 24 * 60 * 60 * 1000);
		});

		for (const appointment of upcomingAppointments) {
			console.log(`Sending appointment reminder to ${appointment.clientEmail}`);
			console.log(`Dear ${appointment.clientName}`);
			console.log(`This is a reminder about your upcomign appoinment with ${appointment.barberName} on ${appointment.appointmentDate.toLocaleDateString()} at ${appointment.appointmentTime}.`);
